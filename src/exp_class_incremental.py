...
def log(msg):
    ...
    print(msg, flush=True)  # Immediate console output with flush
    logger.info(msg)        # Save to file

# Start timing the entire experiment
EXPERIMENT_START_TIME = time.time()

print("=" * 60, flush=True)
print("RSNA Continual Learning Experiment", flush=True)
print("=" * 60, flush=True)
log(f"Timestamp: {timestamp}")
log(f"Log file: {LOG_FILE}")
log(f"Results will be saved to: logs/results_{timestamp}.csv")
...
def set_seed(seed=42):
    ...
    # Set to True for better performance on modern GPUs with fixed input sizes
...
# Disable series filtering - keywords were not matching any series
# Set to None to use all available series
SLICE_KEYWORDS = None

# ============================================================================
# DATA SAMPLING CONFIGURATION
# ============================================================================
# Limit data size for faster experiments while maintaining statistical validity
# Set to None to use all data (will be very slow)
MAX_PATIENTS_PER_CLASS = 4 if DEBUG_RUN else 100  # Use 100 patients per class
MAX_IMAGES_PER_PATIENT = 5 if DEBUG_RUN else 20   # Max 20 images per patient
# This gives ~2000 images per class = ~4000 total, much more manageable
# Device configuration
# Enables Automatic Mixed Precision (AMP) for better performance on modern GPUs
...
if FORCE_CUDA and not torch.cuda.is_available():
    raise RuntimeError(
        "FORCE_CUDA=1 but CUDA is not available. "
        "Verify your PyTorch install and GPU drivers."
    )
...
if device.type == 'cuda':
    try:
        ...
        log(f"GPU memory - allocated: {allocated:.1f} MB, reserved: {reserved:.1f} MB")
...
def load_image_tensor(img_path, target_size, transform=None, window_type=None):
    ...
    if window_type is not None and window_type in CT_WINDOWS:
        window_name, window_func = CT_WINDOWS[window_type]
        img = window_func(img)
...
class RSNADataset(Dataset):
    ...
    def __init__(self, image_paths, labels, target_size=(256, 256), transform=None, window_type=None):
...
class SubDataset(Dataset):
    ...
    def __init__(self, original_dataset, sub_labels):
        ...
        # Use labels directly to avoid loading images just for filtering
        labels = getattr(original_dataset, "labels", None)
        if labels is None:
            raise ValueError("Original dataset must expose labels for efficient filtering.")
        self.indices = [idx for idx, label in enumerate(labels) if label in sub_labels]
    ...
    def get_path_and_label(self, idx):
        ...
        return (
            self.original_dataset.image_paths[original_idx],
            self.original_dataset.labels[original_idx],
        )
...
class MemorySetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        item = self.memory_sets[idx]
        # Support both (sample, label) and (sample, label, window_type) formats
        if len(item) == 3:
            sample, label, window_type = item
        else:
            sample, label = item
            window_type = None
        ...
        if isinstance(sample, torch.Tensor):
            return sample, label
        img_tensor = load_image_tensor(
            sample, self.target_size, self.transform,
...
def load_rsna_data(
    ...
    data_root: data/RSNA2023ProcessedImages
    labels_csv: data/train.csv (uses any_injury label)
    """
    ...
    keyword_tokens = []
    if series_keywords:
        keyword_tokens = [kw.strip().lower() for kw in series_keywords if kw and kw.strip()]
    series_meta_path = DATA_DIR / 'train_series_meta.csv'
    series_description = {}
    if series_meta_path.exists():
        meta_df = pd.read_csv(series_meta_path)
        if 'series_id' in meta_df.columns and 'series_description' in meta_df.columns:
            series_description = {
                str(row['series_id']): str(row['series_description'])
                for _, row in meta_df[['series_id', 'series_description']].dropna().iterrows()
            }
            log(f"Loaded series metadata from: {series_meta_path}")
        else:
            log(f"Series metadata found but missing columns: {series_meta_path}")

    def series_matches(series_dir):
        if not keyword_tokens:
            return False
        name = series_dir.name.lower()
        description = series_description.get(series_dir.name, "").lower()
    ...
    def collect_images(patient_ids, split_name):
        ...
        for pid in patient_ids:
            ...
            if keyword_tokens:
                filtered_series = [d for d in series_dirs if series_matches(d)]
                if filtered_series:
                    matched_series += len(filtered_series)
                    series_dirs = filtered_series
                else:
                    patients_no_match += 1
            ...
            for series_dir in series_dirs:
                ...
                for img_path in series_dir.glob('*.png'):
                    ...
                    if max_images_per_patient is not None and len(patient_images) >= max_images_per_patient:
                        break
                if max_images_per_patient is not None and len(patient_images) >= max_images_per_patient:
                    break
        ...
        stats = {
            ...
            'patients_without_matches': patients_no_match,
            'images': len(image_paths),
        }
        return image_paths, labels, stats

    train_paths, train_labels, train_stats = collect_images(train_patients, "train")
    test_paths, test_labels, test_stats = collect_images(test_patients, "test")

    if not train_paths or not test_paths:
        raise ValueError('Train/test split produced no images. Adjust DEBUG_RUN or test_split.')

    if keyword_tokens:
        log("\nSeries filter enabled:")
        log(f"  Keywords: {', '.join(series_keywords)}")
        for stats in (train_stats, test_stats):
            log(
                f"  {stats['split']} - patients: {stats['patients']}, "
                f"series matched: {stats['matched_series']}/{stats['total_series']}, "
                f"patients without matches: {stats['patients_without_matches']}"
            )

    log(f"\nLoaded {len(train_paths)} training images")
    log(f"Loaded {len(test_paths)} test images")
    log(f"Number of classes: {len(set(train_labels))}")
...
def create_task_datasets(base_dataset, num_tasks):
    ...
    if labels is None:
        raise ValueError("Base dataset must expose labels to create tasks.")
    ...
    for task_id in range(num_tasks):
        ...
        log(f"Task {task_id+1}: Classes {task_labels}, {len(task_dataset)} samples")
...
def create_window_task_datasets(image_paths, labels, target_size, num_tasks=3):
    ...
    Task 0: Brain Window
    Task 1: Lung Window
    Task 2: Soft Tissue Window

    Each task uses the SAME images but with DIFFERENT windowing applied.
    This simulates learning from different imaging protocols sequentially.
    """
    task_datasets = []

    for task_id in range(num_tasks):
        window_name = CT_WINDOWS[task_id][0]
        # Create dataset with specific window type
        task_dataset = RSNADataset(
...
def fill_memory_buffer(dataset, buffer_size_per_task, window_type=None):
    """Fill memory buffer with samples from a task (window type)."""
    memory_sets = []

    indices = list(range(len(dataset)))
    if len(indices) > buffer_size_per_task:
        indices = random.sample(indices, buffer_size_per_task)

    for idx in indices:
        if hasattr(dataset, "image_paths") and hasattr(dataset, "labels"):
            image = dataset.image_paths[idx]
            label = dataset.labels[idx]
        else:
            # Fallback - but we avoid loading images here
            continue

        # Store with window_type for replay
...
train_paths, train_labels, test_paths, test_labels = load_rsna_data(
    ...
    series_keywords=SLICE_KEYWORDS,
    seed=42,
)
...
log(f"Test dataset: {len(test_dataset)} samples")
...
def train_baseline(model, dataset, iters, lr, batch_size, device):
    ...
    # Use pin_memory=True and non_blocking=True for faster data transfer to the GPU.
...
def evaluate_accuracy(model, dataset, device, batch_size, num_workers=None):
    ...
    log(
        f"  Evaluating {len(dataset)} samples | Batch size: {batch_size} | "
    ...
    with torch.no_grad():
        ...
        for images, labels in progress_bar:
            ...
            if total > 0:
                progress_bar.set_postfix({'acc': f'{(100.0 * correct / total):.2f}%'})
...
# Increased iterations to compensate for larger batch size
# With batch_size=32 and ~1600 samples/task, one epoch = ~50 iterations
# 1000 iterations = ~20 epochs per task for better convergence
...
EVAL_NUM_WORKERS = 0 if os.name == 'nt' else (os.cpu_count() or 0)
...
BUFFER_SIZE_PER_CLASS = 500 if not DEBUG_RUN else 5
...
log(f"  Eval workers: {EVAL_NUM_WORKERS}")
log(f"  EWC lambda: {EWC_LAMBDA}")
log(f"  Replay buffer/class: {BUFFER_SIZE_PER_CLASS}")
log(f"  Max patients/class: {MAX_PATIENTS_PER_CLASS}")
log(f"  Max images/patient: {MAX_IMAGES_PER_PATIENT}")
if SLICE_KEYWORDS:
    log(f"  Series keywords: {', '.join(SLICE_KEYWORDS)}")
...
for task_id in range(NUM_TASKS):
    ...
    for eval_task_id in range(NUM_TASKS):
        acc = evaluate_accuracy(
            model_baseline,
            test_task_datasets[eval_task_id],
            device,
            batch_size=EVAL_BATCH_SIZE,
        )
        baseline_matrix[task_id, eval_task_id] = acc
        log(f"  Task {eval_task_id + 1}: {acc:.2f}%")
...
del model_baseline  # Free memory from previous experiment
clear_gpu_memory()
...
del model_ewc  # Free memory from previous experiment
clear_gpu_memory()
...
for task_id in range(NUM_TASKS):
    ...
    buffer_dataset = (
        MemorySetDataset(memory_buffer, train_dataset.target_size, train_dataset.transform)
...
del model_replay  # Free memory from previous experiment
clear_gpu_memory()
...
for task_id in range(NUM_TASKS):
    ...
    buffer_dataset = (
        MemorySetDataset(memory_buffer_combined, train_dataset.target_size, train_dataset.transform)
...
results_csv = LOG_DIR / f'results_{timestamp}.csv'
