...
LOG_FILE = LOG_DIR / f'experiment_v3_windows_{timestamp}.log'
...
print("=" * 70, flush=True)
print("RSNA Continual Learning - 3-Task CT Window Experiment (V3)", flush=True)
print("=" * 70, flush=True)
log(f"Timestamp: {timestamp}")
log(f"Log file: {LOG_FILE}")
log(f"Results will be saved to: logs/results_v3_windows_{timestamp}.csv")
...
def create_window_task_datasets(image_paths, labels, target_size, num_tasks=3, transform=None):
    """
    ...
    for task_id in range(num_tasks):
        ...
        task_dataset = RSNAWindowDataset(
            image_paths, labels, target_size=target_size, transform=transform, window_type=task_id
        )
...
def train_baseline(model, dataset, iters, lr, batch_size, device):
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=iters, eta_min=lr * 0.1
    )
...
# Data transforms
# Medical CT slices: allow mild geometric augmentation, avoid vertical flips
# and heavy color jitter. Normalize to stabilize training.
...
train_task_datasets = create_window_task_datasets(
    train_paths, train_labels, TARGET_SIZE, NUM_TASKS, transform=TRAIN_TRANSFORM
)
test_task_datasets = create_window_task_datasets(
    test_paths, test_labels, TARGET_SIZE, NUM_TASKS, transform=EVAL_TRANSFORM
)
...
if not DEBUG_RUN:
    plot_forgetting_matrix(baseline_matrix, 'Baseline', WINDOW_NAMES,
                          LOG_DIR / f'forgetting_baseline_v3_{timestamp}.png')
    plot_forgetting_matrix(ewc_matrix, 'EWC', WINDOW_NAMES,
                          LOG_DIR / f'forgetting_ewc_v3_{timestamp}.png')
    plot_forgetting_matrix(replay_matrix, 'Replay', WINDOW_NAMES,
                          LOG_DIR / f'forgetting_replay_v3_{timestamp}.png')
    plot_forgetting_matrix(combined_matrix, 'EWC+Replay', WINDOW_NAMES,
                          LOG_DIR / f'forgetting_combined_v3_{timestamp}.png')
...
results_csv = LOG_DIR / f'results_v3_windows_{timestamp}.csv'
results_df.to_csv(results_csv, index=False)

# Save detailed matrices
np.savez(LOG_DIR / f'matrices_v3_windows_{timestamp}.npz',
...
log(f"Matrices saved to: logs/matrices_v3_windows_{timestamp}.npz")
